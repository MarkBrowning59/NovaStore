...
// Stable empty object to avoid new reference creation on each render
const EMPTY_CATALOG_NODES = Object.freeze({});
...
export default function CatalogsPageBase({ renderLayout }) {
  ...
  const [statusFilter, setStatusFilter] = useState(() => {
    try { return localStorage.getItem(LS_KEYS.status) || '1'; } catch { return '1'; }
  ...
  const catalogNodes = state.catalogNodes ?? EMPTY_CATALOG_NODES;
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LS_KEYS.search, searchQuery ?? '');
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(LS_KEYS.status, statusFilter ?? 'all');
    } catch (e) {
      // ignore (e.g., storage disabled)
    }
  }, [statusFilter]);
