...
function getStatusMeta(statusId) {
  ...
  switch (s) {
    ...
    case '0':
      return { label: 'Inactive', cls: 'bg-gray-100 text-gray-700 border-gray-200' };
    case '3':
      return { label: 'Archived', cls: 'bg-amber-100 text-amber-800 border-amber-200' };
    default:
      return { label: s || 'Unknown', cls: 'bg-gray-100 text-gray-700 border-gray-200' };
  }
}
...
export default function CatalogList({
  ...
  onCatalogClick,
  onEditCatalog,
  onReorder, // (orderedCatalogs) => void
  setLastItem,
}) {
  ...
  return (
    <div className="space-y-1">
      ...
      {rows.map((catalog, index) => {
        ...
        const status = getStatusMeta(catalog?.StatusID ?? catalog?.statusID);
